...
var castling_permission : Array = [[false, false], [false, false]] # King / Queen side
...
func get_valid_pawn_placements() -> Array:
	var moves = [];
	var rows = [];
	if player_turn == Globals.PieceColor.BLACK:
		rows = [1,2]
	else:
		rows = [5,6]
	for row in rows:
...
func get_valid_nonpawn_placements() -> Array:
	var moves = [];
	var rows = [];
	if player_turn == Globals.PieceColor.BLACK:
		rows = [0,1]
