...
var game_state : Globals.GameState = Globals.GameState.DRAFTING
var board : Board
var markers : Array = []
...
var start_turn_time : int = 0
var clock_times : Array[int] = [60 * 30, 60 * 30]

var selected_tile : Coord
var selected_piece : Piece

var purchased_piece : int = -1
var valid_purchase_places : Array = []
var purchase_fen_str : String = "";
...
func _process(delta):
	...
	if game_state == Globals.GameState.PLAYING or game_state == Globals.GameState.DRAFTING:
...
func set_purchase_fen_str(piece_type):
	if piece_type == Globals.PieceType.PAWN:
		purchase_fen_str = "p"
	if piece_type == Globals.PieceType.KNIGHT:
		purchase_fen_str = "n"
	if piece_type == Globals.PieceType.BISHOP:
		purchase_fen_str = "b"
	if piece_type == Globals.PieceType.ROOK:
		purchase_fen_str = "r"
	if piece_type == Globals.PieceType.QUEEN:
		purchase_fen_str = "q"
	if board.get_player_turn() == Globals.PieceColor.WHITE:
		purchase_fen_str = purchase_fen_str.to_upper()
	
func purchase_markers():
	reset_markers()
	valid_purchase_places = [];
	if purchased_piece == Globals.PieceType.PAWN:
		valid_purchase_places = board.get_valid_pawn_placements()
	if (purchased_piece == Globals.PieceType.KNIGHT or purchased_piece == Globals.PieceType.BISHOP 
		or purchased_piece == Globals.PieceType.ROOK or purchased_piece == Globals.PieceType.QUEEN):
		valid_purchase_places = board.get_valid_nonpawn_placements()
	
	
	for coord in valid_purchase_places:
		var row = coord[0]
		var col = coord[1]
		markers[row][col].set_color(Color.GREEN)
		markers[row][col].visible = true;
...
func handle_click(boundingRectangle, pos):
	...
	if game_state == Globals.GameState.DRAFTING:
		if [tile_y, tile_x] in valid_purchase_places:
			create_piece_from_info(new_selected_coord, Globals.piece_info_from_fen_string(purchase_fen_str))
			place_piece_visuals()
			$Shop.handle_purchase(purchase_fen_str)
		reset_markers()
		purchase_fen_str = ""
		valid_purchase_places = []
...
func begin_draft():
	game_state = Globals.GameState.DRAFTING
	$Shop.reset_shop()
	board.set_player_turn(Globals.PieceColor.BLACK)
	$UI/Clock/WhiteTurnMarker.visible = false
	$UI/Clock/BlackTurnMarker.visible = true
	var king_col = randi() % 8
	create_piece_from_info(Coord.new(1, Coord.file_from_col(king_col)), Globals.piece_info_from_fen_string('K'))
	create_piece_from_info(Coord.new(8, Coord.file_from_col(king_col)), Globals.piece_info_from_fen_string('k'))
	place_piece_visuals()
